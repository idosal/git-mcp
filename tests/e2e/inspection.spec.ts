  ...
  test.describe("SSE", () => {
    ...
    for (const testCase of testCases) {
      ...
      test(`should list tools for ${path}`, async ({ page }) => {
        await page.goto("/");
        await page.getByRole("combobox", { name: /Transport Type|Transport/i }).click();
        await page.getByRole("option", { name: /SSE/i }).click();
        await page.getByRole("textbox", { name: "URL" }).fill(targetServerUrl);
        await page.getByRole("button", { name: "Connect" }).click();

        const toolsButton = page.getByRole("button", { name: /List Tools|Get Tools|Tools/i });
        await expect(toolsButton).toBeVisible({ timeout: 15000 });
        await toolsButton.click();

        await expect(page.getByText("fetch_generic_url_content")).toBeVisible({ timeout: 20000 });
        if (path === 'mrdoob/three.js') {
          //  await expect(page.getByText('fetch_threejs_documentation')).toBeVisible({ timeout: 5000 });
           ...
           await expect(page.getByText('search_threejs_documentation')).toBeVisible({ timeout: 20000 });
        } else {
           // Check for generic fetch and search tools
           await expect(page.getByText(/^fetch_.*_documentation$/)).toBeVisible({ timeout: 20000 });
           await expect(page.getByText(/^search_.*_documentation$/)).toBeVisible({ timeout: 20000 });
        }
      });
      ...
      test(`should run fetch_X_documentation for ${path}`, async ({ page }) => {
         ...
         const fetchToolButton = page.getByText(/^fetch_.*_documentation$/);
          await expect(fetchToolButton).toBeVisible({ timeout: 20000 });
